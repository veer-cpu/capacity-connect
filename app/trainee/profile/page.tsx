...
export default async function TraineeProfilePage() {
  ...
  const { data: traineeProfile } = await supabase
    ...
    .eq("user_id", user.id)
    .maybeSingle();
  const [professionalProfile, certificates] = await Promise.all([
    ...
    <main className="space-y-8">
      ...
      <ProfileForm
        initialData={{
          ...
          employee_code: traineeProfile?.employee_code ?? "",

          qualifications: traineeProfile?.qualifications ?? "",

          work_experience: traineeProfile?.work_experience ?? "",

          professional_interests: traineeProfile?.professional_interests ?? "",
        }}
      />
      <Separator />
      <ProfessionalProfile
        profile={professionalProfile}
        profilePath="/trainee/profile"
