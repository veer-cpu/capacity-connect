...
export async function getMyCompetencyPassport():
  Promise<CompetencyPassportItem[]> {
  await requireRole("trainee");

  const supabase = await createClient();

  const { data, error } = await supabase.rpc(
    "get_my_competency_passport",
  );

  if (error) {
    console.error(
      "Unable to load competency passport:",
      {
  ...
  return (data ?? []).map(
    (row: PassportRpcRow) => ({
      ...
      proficiencyBand:
        row.proficiency_band,

      assessmentEvidenceCount:
        Number(
          row.assessment_evidence_count,
        ),
      ...
      latestAssessmentPercentage:
        row.latest_assessment_percentage === null
          ? null
          : Number(
              row.latest_assessment_percentage,
            ),

      latestAssessmentAt:
        row.latest_assessment_at,

      scoreHistoryCount:
        Number(row.score_history_count),

      previousScore:
        row.previous_score === null
          ? null
          : Number(row.previous_score),

      improvement:
        row.improvement === null
          ? null
          : Number(row.improvement),

      lastScoreUpdate:
        row.last_score_update,

      certificateCount:
        Number(row.certificate_count),
    }),
  );
}